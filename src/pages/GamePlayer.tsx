...
export const GamePlayer: React.FC = () => {
  ...
  const BoardSection = (
    <div style={{ position: 'relative', flexShrink: 0 }}>
      <div style={{ 
        ...
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: borderThick*1.2, background: 'linear-gradient(to right, #00b09b, #96c93d)', zIndex: 2 }} />
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: borderThick*1.2, background: 'linear-gradient(to right, #00b09b, #96c93d)', zIndex: 2 }} />
          ...
          {syncedBoard.map(hex => {
            ...
            return (
              <div key={hex.id} style={{
                ...
                <span style={{ 
                  fontSize: `${hexSize * 0.35}px`, fontWeight: '800', 
  ...
  return (
    <div ref={containerRef} style={{
      ...
      background: `radial-gradient(circle at 50% 0%, ${teamColor}15 0%, rgb(8,8,14) 75%)`,
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: `${teamColor}22`, border: `2px solid ${teamColor}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1rem', fontWeight: '900', color: teamColor }}>
            {myName?.[0] ?? '?'}
          </div>
          <div>
            <div style={{ fontSize: '0.95rem', fontWeight: '900', lineHeight: 1.1 }}>{myName}</div>
            <div style={{ fontSize: '0.7rem', color: teamColor, fontWeight: '700' }}>{teamLabel}</div>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <button onClick={toggleWakeLock} style={{ background: wakeLockActive ? 'rgba(255,180,0,0.15)' : 'rgba(255,255,255,0.05)', border: `1px solid ${wakeLockActive ? 'rgba(255,180,0,0.3)' : 'rgba(255,255,255,0.1)'}`, borderRadius: '8px', padding: '5px 10px', color: wakeLockActive ? '#ffb400' : '#666', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px', fontSize: '0.7rem' }}>
            {wakeLockActive ? <Sun size={14} fill="currentColor" /> : <Moon size={14} />}
            <span>{wakeLockActive ? 'Active' : 'Wake'}</span>
          </button>
          <div style={{ background: 'rgba(255,107,107,0.12)', border: '1px solid rgba(255,107,107,0.25)', borderRadius: '8px', padding: '3px 12px', textAlign: 'center' }}>
            <div style={{ fontSize: '0.5rem', color: '#ff6b6b', fontWeight: '800' }}>ROOM</div>
            <div style={{ fontSize: '1.1rem', fontWeight: '950', letterSpacing: '3px', color: '#ff6b6b', lineHeight: 1 }}>{roomCode}</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: isLandscape ? 'row' : 'column', alignItems: 'center', gap: '20px', padding: '15px', overflow: 'hidden' }}>
        ...
        {/* Right Info Section */}
        <div style={{ flex: 1, width: '100%', height: '100%', display: 'flex', flexDirection: 'column', gap: '12px', justifyContent: 'center', minWidth: 0 }}>
          
          {/* Scoreboard (Smaller Size) */}
          <div style={{ display: 'flex', gap: '8px' }}>
            {[
              { rounds: syncedTeam1Rounds, color: '#ff416c', bg: 'rgba(255,65,108,0.15)', border: 'rgba(255,65,108,0.25)', label: 'أحمر' },
              { rounds: syncedTeam2Rounds, color: '#00b09b', bg: 'rgba(0,176,155,0.15)', border: 'rgba(0,176,155,0.25)', label: 'أخضر' },
            ].map(({ rounds, color, bg, border, label }) => (
              <div key={label} style={{ flex: 1, textAlign: 'center', background: bg, border: `1px solid ${border}`, borderRadius: '10px', padding: '6px 0' }}>
                <div style={{ fontSize: '1.5rem', fontWeight: '900', color, lineHeight: 1 }}>{rounds}</div>
                <div style={{ fontSize: '0.7rem', color, opacity: 0.8, fontWeight: '700' }}>{label}</div>
              </div>
            ))}
          </div>

          <div style={{ textAlign: 'center', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '12px', padding: '10px' }}>
            <div style={{ fontSize: '0.85rem', color: '#888' }}>دور الاختيار:</div>
            <div style={{ color: syncedTurn === 'team1' ? '#ff416c' : '#00b09b', fontWeight: '900', fontSize: '1.1rem' }}>
              {syncedTurn === 'team1' ? 'الفريق الأحمر' : 'الفريق الأخضر'}
            </div>
          </div>

          {/* Spacer if no question */}
          {!questionActive && <div style={{ flex: 1 }} />}

          {/* Buzz Result Status (Persistent but small) */}
          ...
          {isSyncing && !firstBuzz && (
            <div style={{ textAlign: 'center', color: '#ffb400', fontSize: '0.85rem', fontWeight: '700', animation: 'pulse 1s infinite' }}>جاري المزامنة...</div>
          )}

          {/* Buzz Button (Fixed Position) */}
          <button
            onClick={handleBuzz}
            ...
            style={{
              ...
              border: questionActive && !buzzed ? '1px solid rgba(255,255,255,0.15)' : '1px solid rgba(255,255,255,0.06)',
              color: !questionActive ? '#444' : buzzed ? '#666' : 'white',
              fontSize: '1.8rem', fontWeight: '950', cursor: buzzed ? 'default' : 'pointer',
              boxShadow: questionActive && !buzzed ? `0 12px 35px ${teamColor}55` : 'none',
      ...
      {/* ── Question Modal Overlay (Fixes Overflow) ── */}
      {questionActive && currentQuestion && (
        <div style={{
          position: 'fixed', top: 0, left: 0, right: 0, bottom: 0,
          background: 'rgba(0,0,0,0.82)', backdropFilter: 'blur(10px)',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          padding: '20px', zIndex: 1000, animation: 'fadeIn 0.3s ease-out'
        }}>
          <div style={{
            background: 'linear-gradient(135deg, #2a2c3a, #1a1c2a)',
            border: '1px solid rgba(255,255,255,0.15)', borderRadius: '24px',
            padding: '24px', width: '100%', maxWidth: '480px', textAlign: 'center',
            boxShadow: '0 25px 60px rgba(0,0,0,0.5)', animation: 'popUp 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275)'
          }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', marginBottom: '16px' }}>
              <div style={{ width: '40px', height: '40px', background: 'rgba(255,210,0,0.1)', border: '1px solid #ffd200', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#ffd200', fontSize: '1.4rem', fontWeight: '900' }}>
                {currentQuestion.letter}
              </div>
              <div style={{ color: '#aaa', fontSize: '0.9rem', fontWeight: '700' }}>سؤال الحرف</div>
            </div>
            
            <div style={{ 
              maxHeight: '40vh', overflowY: 'auto', marginBottom: '20px', padding: '0 10px',
              scrollbarWidth: 'thin', scrollbarColor: 'rgba(255,255,255,0.1) transparent'
            }}>
              <p style={{ fontSize: '1.25rem', fontWeight: '800', color: 'white', lineHeight: 1.6, margin: 0 }}>
                {currentQuestion.question}
              </p>
            </div>

            {answerRevealed && (
              <div style={{ background: 'rgba(247,151,30,0.12)', border: '1px solid rgba(247,151,30,0.3)', borderRadius: '16px', padding: '15px', animation: 'fadeIn 0.5s' }}>
                <div style={{ fontSize: '0.75rem', color: '#f7971e', fontWeight: '800', marginBottom: '4px' }}>الجواب</div>
                <div style={{ fontSize: '1.4rem', fontWeight: '950', color: '#ffd200' }}>{revealedAnswer}</div>
                {awardedTeam && awardedTeam !== 'none' && (
                  <div style={{ marginTop: '10px', fontSize: '1rem', color: awardedTeam === 'team1' ? '#ff6b6b' : '#00d4b4', fontWeight: '900', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
                    <Zap size={16} /> {awardedTeam === 'team1' ? 'نقطة للأحمر!' : 'نقطة للأخضر!'}
                  </div>
                )}
              </div>
            )}
            
            <div style={{ marginTop: '20px', color: '#666', fontSize: '0.8rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '5px' }}>
              <Info size={12} /> اضغط زر Buzz في الخلفية للإجابة
            </div>
          </div>
        </div>
      )}
